...
class Codec(object):
    ...
    def encode(self, msg, session):
        ...
        body.append(
            "%s=%s" % (self.protocol.fixtags.SenderCompID, session.senderCompId)
        )
        body.append(
            "%s=%s" % (self.protocol.fixtags.TargetCompID, session.targetCompId)
        )
        ...
        if msgType == self.protocol.msgtype.SEQUENCERESET:
            if (
                ...
                try:
                    ...
                    raise EncodingError(
                        "SequenceReset with GapFill='Y' must have the MsgSeqNum already populated"
                    )
            else:
            ...
            if (
                ...
                try:
                    ...
                    raise EncodingError(
                        "Failed to encode message with PossDupFlay=Y but no previous MsgSeqNum"
                    )
            else:
        ...
        header.append(
            "%s=%i" % (self.protocol.fixtags.BodyLength, len(body) + len(msgType) + 1)
        )
    ...
    def decode(self, rawmsg: bytes) -> Tuple[str, str]:
        ...
        try:
            ...
            if (
                len(msg) < 3
            ):  # at a minumum we require BeginString, BodyLength & Checksum
                return (None, 0)

            tag, value = msg[0].split("=", 1)
            if tag != self.protocol.fixtags.BeginString:
                logging.error(
                    "*** BeginString missing or not 1st field *** [" + tag + "]"
                )
            ...
            if tag != self.protocol.fixtags.BodyLength:
                logging.error(
                    "*** BodyLength missing or not 2nd field *** [" + tag + "]"
                )
            else:
                msgLength += int(value)
            ...
            if msgLength > len(rawmsg):
                ...
                for m in msg:
                    ...
                    if (
                        tag in repeatingGroupTags
                    ):  # found the start of a repeating group
                        if (
                            type(currentContext) is RepeatingGroupContext
                        ):  # i.e. we are already in a repeating group
                            while (
                                repeatingGroups
                                and tag not in currentContext.repeatingGroupTags
                            ):
                                currentContext.parent.addRepeatingGroup(
                        ...
                        ctx = RepeatingGroupContext(
                            tag, repeatingGroupTags[tag], currentContext
                        )
                        ...
                        if tag in currentContext.tags:
                            ...
                            ctx = RepeatingGroupContext(
                                currentContext.tag,
                                currentContext.repeatingGroupTags,
